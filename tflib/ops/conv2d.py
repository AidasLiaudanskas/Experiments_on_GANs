...
def Conv2D(name, input_dim, output_dim, filter_size, inputs, he_init=True, mask_type=None, stride=1, weightnorm=None, biases=True, gain=1.):
    ...
    """
    # output_dim = int(output_dim) # Due to Python 2to3 differences with int division
    # print("Conv beginning, inputs shape: ", inputs.shape)
    with tf.name_scope(name) as scope:
        ...
        # again NCHW --> NHWC: Might need to adjust filters as well.
        if FLAGS.data_format == "NHWC":
            strides = [1, stride, stride, 1]
        # print("Inputs, filters: ", inputs, filters)
        result = tf.nn.conv2d(
            input=inputs,
            filter=filters,
            strides=strides,
        ...
        if biases:
            ...
            result = tf.nn.bias_add(result, _biases, data_format=FLAGS.data_format)
