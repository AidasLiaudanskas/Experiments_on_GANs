...
def Batchnorm(name, axes, inputs, is_training=None, stats_iter=None, update_moving_stats=True, fused=True):
    # Quick fix for CPU testing NHWC --> NCHW:
    if FLAGS.data_format == "NCHW" and inputs.shape[3] in [1,3]:
    ...
    if ((axes == [0, 2, 3]) or (axes == [0, 2])) and fused == True:
        ...
        moving_mean = lib.param(name + '.moving_mean', np.zeros(inputs.get_shape()
                                                                [1], dtype='float32'), trainable=False)
        moving_variance = lib.param(name + '.moving_variance', np.ones(inputs.get_shape()
                                                                       [1], dtype='float32'), trainable=False)

        def _fused_batch_norm_training():
            # print("Entering batch norm:")
            # print("Inputs shape: ", inputs.shape)
            output, mean, var = tf.nn.fused_batch_norm(inputs, scale, offset, epsilon=1e-5, data_format=FLAGS.data_format)
            # print("Output shape: ", output.shape)
            if FLAGS.data_format == "NHWC" and output.shape[1] in [1,3]:
                return tf.transpose(output, perm=[0, 2, 3, 1]), mean, var
            else:
        ...
        def _fused_batch_norm_inference():
            ...
            output = tf.nn.batch_normalization(
                inputs, mean, var, offset[None, :, None, None], scale[None, :, None, None], 1e-5), mean, var
            if FLAGS.data_format == "NCHW" and inputs.shape[3] in [1,3]:
                return tf.transpose(output, perm=[0, 3, 1, 2])
            else:
                return output
            # Standard version
            ...
            if update_moving_stats:
                ...
                def _force_updates():
                    ...
                    update_moving_mean = tf.assign(
                        moving_mean, ((float_stats_iter / (float_stats_iter + 1)) * moving_mean) + ((1 / (float_stats_iter + 1)) * batch_mean))
                    update_moving_variance = tf.assign(moving_variance, ((
                        float_stats_iter / (float_stats_iter + 1)) * moving_variance) + ((1 / (float_stats_iter + 1)) * batch_var))
        ...
        output = tf.nn.batch_normalization(inputs, mean, var, offset, scale, 1e-5)
        if FLAGS.data_format == "NCHW" and inputs.shape[3] in [1,3]:
