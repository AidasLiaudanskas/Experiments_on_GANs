...
def featurize(images, model, batch_size=100, transformer=np.asarray,
    ...
    if isinstance(model, Inception):
        ...
        if (sub.max() > 255) or (sub.min() < 0):
            print('WARNING! Inception min/max violated: min = %f, max = %f. Clipping values.' %
                  (sub.min(), sub.max()))
            sub = sub.clip(0., 255.)
        ...
        if (sub.max() > .5) or (sub.min() < -.5):
            print('WARNING! LeNet min/max violated: min = %f, max = %f. Clipping values.' %
                  (sub.min(), sub.max()))
            sub = sub.clip(-.5, .5)
...
def polynomial_mmd_averages(codes_g, codes_r, n_subsets=50, subset_size=1000,
                            ret_var=False, output=sys.stdout, **kernel_args):
    """
    I believe the Inputs are:
    codes_g - activations at pool3 layer of inception net when feeding in fake images
    codes_r - activations at pool3 layer of inception net when feeding in real images
...
def main():
    ...
    if args.model == 'inception':
        ...
        if samples.dtype == np.uint8:
            ...
            def transformer(x): return x * 255
        else:
            raise TypeError(
                "don't know how to handle {}".format(samples.dtype))
    ...
    print('Transformer test: transformer([-1, 0, 10.]) = ' +
          repr(transformer(np.array([-1, 0, 10.]))))
